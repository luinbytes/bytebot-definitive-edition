...
class TicketService {
    ...
    getPanel(guildId, nameOrId) {
        return typeof nameOrId === 'number' || /^\d+$/.test(String(nameOrId))
            ? this.sqlite.prepare('SELECT * FROM ticket_panels WHERE guild_id = ? AND id = ?').get(guildId, Number(nameOrId))
    ...
    addFormField(formId, values) {
        ...
        return this.sqlite.prepare(`INSERT INTO ticket_form_fields (form_id, label, type, placeholder, required, position)
            VALUES (?, ?, ?, ?, ?, ?) RETURNING *`).get(formId, String(values.label || '').slice(0, 45), values.type || 'short',
    ...
    async publishPanel(guild, panel, channel) {
        ...
        if (panel.message_id && panel.channel_id) {
            const oldChannel = guild.channels.cache.get(panel.channel_id);
            const oldMessage = await oldChannel?.messages?.fetch?.(panel.message_id).catch(() => null);
            if (oldMessage) await oldMessage.edit({ components: [] }).catch(() => null);
        }
        const message = await channel.send({
            embeds: [new EmbedBuilder().setTitle(panel.name).setDescription('Choose an option below to open a support ticket.')],
            components: this.panelComponents(panel.id), allowedMentions: SAFE_MENTIONS
        });
    ...
    async openTicket(interaction, optionId, formSnapshot = null) {
        ...
        const ticket = this.reserveTicket({
            ...
            accessSnapshot: { supportRoleIds: supportIds, traineeRoleIds: traineeIds, traineeClaim: Boolean(option.trainee_claim) }
        });
        const overwrites = [
    ...
    async log(guildId, content, files = []) {
        ...
        return channel.send({ content, files, allowedMentions: SAFE_MENTIONS });
    }
    ...
    async closeDiscordTicket(ticket, channel, actorId, reason) {
        ...
        const members = [{ target_type: 'member', target_id: ticket.openerId },
            ...this.sqlite.prepare('SELECT target_type, target_id FROM ticket_members WHERE ticket_id = ?').all(ticket.id)];
        for (const target of members) {
            await channel.permissionOverwrites.edit(target.target_id, { SendMessages: false }, { reason: `Ticket #${ticket.number} closed` });
        }
    ...
    async reopenDiscordTicket(ticket, channel, actorId) {
        ...
        const members = [{ target_type: 'member', target_id: ticket.openerId },
            ...this.sqlite.prepare('SELECT target_type, target_id FROM ticket_members WHERE ticket_id = ?').all(ticket.id)];
        for (const target of members) {
            await channel.permissionOverwrites.edit(target.target_id, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }, { reason: `Ticket #${ticket.number} reopened` });
        }
    ...
    async deleteDiscordTicket(ticket, channel, actorId, reason = null) {
        ...
        try {
            ...
            this.sqlite.prepare("UPDATE tickets SET status = 'closed', updated_at = ? WHERE id = ? AND status = 'deleting'").run(this.now(), ticket.id);
            throw error;
        }
        try {
    ...
    managerPayload(panel) {
        ...
        return {
            content: `**${panel.name}** · ${panel.mode}\nOptions: ${options.length ? options.map(option => `#${option.id} ${option.label}`).join(', ') : 'none'}\nForms: ${forms.length ? forms.map(form => `#${form.id} ${form.name}`).join(', ') : 'none'}`,
    ...
    async executeCommand(interaction) {
        ...
        if (action === 'setup' && !group) {
            ...
            if (!this.listOptions(panel.id).length) this.addOption(panel.id, { label: 'Create ticket', style: 'primary' });
        ...
        if (action === 'support' && !group) {
            this.updateConfig(guildId, { supportRoleId: this.option(interaction, 'getRole', 'role').id });
        ...
        if (action === 'button' && !group) {
            this.updateConfig(guildId, { buttonLabel: this.option(interaction, 'getString', 'label'), buttonStyle: this.option(interaction, 'getString', 'style') || 'primary' });
        ...
        if (group === 'panel') {
            ...
            if (action === 'list') {
                ...
                return interaction.editReply({ content: panels.length ? panels.map(panel => `#${panel.id} **${panel.name}** · ${panel.mode} · ${this.listOptions(panel.id).length} options${panel.message_id ? ` · <#${panel.channel_id}>` : ''}`).join('\n') : 'No ticket panels are configured.', allowedMentions: SAFE_MENTIONS });
            }
    ...
    async handleTopicCommand(interaction, action) {
        ...
        if (action === 'list') {
            ...
            return interaction.editReply({ content: topics.length ? topics.map(topic => `#${topic.id} **${topic.name}**${topic.category_id ? ` · <#${topic.category_id}>` : ''}`).join('\n') : 'No ticket topics are configured.', allowedMentions: SAFE_MENTIONS });
        }
    ...
    handleSettingsCommand(interaction, action) {
        ...
        const field = {
            ...
            logs: ['logChannelId', this.option(interaction, 'getChannel', 'channel')?.id || null],
            rating: ['ratingsEnabled', this.option(interaction, 'getBoolean', 'enabled')],
            vouch: ['vouchChannelId', this.option(interaction, 'getChannel', 'channel')?.id || null]
        }[action];
    ...
    handleAccessCommand(interaction, action) {
        ...
        if (action === 'list') {
            ...
            return interaction.editReply({ content: rows.length ? rows.map(row => row.target_type === 'member' ? `<@${row.target_id}>` : `<@&${row.target_id}>`).join('\n') : 'The ticket opening blacklist is empty.', allowedMentions: SAFE_MENTIONS });
        }
    ...
    listCommand(interaction) {
        ...
        return interaction.editReply({ content: rows.length ? rows.map(ticket => `#${ticket.number} · **${ticket.status}** · ${ticket.channelId ? `<#${ticket.channelId}>` : 'no channel'} · <@${ticket.openerId}>`).join('\n') : 'No accessible tickets matched.', allowedMentions: SAFE_MENTIONS });
    }
    ...
    async handleRating(interaction, ticketId, stars) {
        const ticket = this.getTicket(ticketId);
        if (!ticket || ticket.openerId !== interaction.user.id || stars < 1 || stars > 5) throw new Error('That rating prompt is invalid.');
        this.sqlite.prepare(`INSERT INTO ticket_ratings (ticket_id, user_id, stars, created_at) VALUES (?, ?, ?, ?)
            ON CONFLICT (ticket_id) DO UPDATE SET stars = excluded.stars, created_at = excluded.created_at`)
            .run(ticketId, interaction.user.id, stars, this.now());
        const config = this.getConfig(ticket.guildId);
    ...
    async handleMemberRemove(member) {
        const rows = this.sqlite.prepare(`SELECT t.* FROM tickets t JOIN ticket_options o ON o.id = t.option_id
            WHERE t.guild_id = ? AND t.opener_id = ? AND t.status IN ('open', 'claimed') AND o.close_on_leave = 1`).all(member.guild.id, member.id);
        for (const row of rows) {
            ...
            if (channel) await this.closeDiscordTicket(ticket, channel, this.client.user.id, 'Ticket opener left the server');
        }
    }
