...
| Dashboard, Commands, Embeds product routes | #34, #44, `greed-parity.md`, `greed-rich-content-contract.md` | Commands and embeds map to ByteBot help and bounded rich-content paths. ByteBot has no web dashboard; dashboard-only Greed controls are an explicit product-surface boundary, not a hidden bot command. |
