...
Welcome and its optional DM run only after AntiRaid and join-time AutoMod checks. Welcome and goodbye ignore bots; Welcome pauses after 20 accepted human joins in a rolling minute. Both attempt every configured destination independently. Goodbye tolerates the partial member snapshot Discord provides on removal. Boost fires only on the inactive-to-active premium transition. Test messages use the same renderer and sender as those events.
